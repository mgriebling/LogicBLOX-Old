...
- (void)drawPathsFromPosition:(CGFloat)oldPosition toPosition:(CGFloat)position animationDuration:(CFTimeInterval)duration completion:(void (^)(void))completion
{
    ...
    __block void(^animationCompletion)(void);
    if (completion)
    {
        animationCompletion = ^
        {
...
+ (id)appearance
{
//    return [self appearanceWhenContainedInInstancesOfClasses:SDSegmentedControl.class];
